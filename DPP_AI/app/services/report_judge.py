...
4) 포맷: 플레인 텍스트 3단 구성(오늘의 사용 요약 / 체크인 돌아보기 / 내일을 위한 한 가지)이 빠지지 않았는지. 앱은 마크다운 미지원이므로 #, **, `, "- " 목록 등 마크다운 기호가 본문에 남아 있으면 REWRITE다.
   시간을 시·분·초로 쓴 뒤 괄호에 원본 초만 덧붙인 표기(예: "50분 30초(3030초)", "44분(2,675초)")는 불필요하므로 있으면 REWRITE다.
...
def run_report_judge(
    ...
    *,
    ...
    Args:
        report_draft: 리포트 초안(플레인 텍스트 권장, 필드명은 호환용)
        snapshot: 오늘 스냅샷
        kpt: KPT 컨텍스트
