...
- [시간 단위 변환] observation·interpretation 필드에서 초(sec/second/초) 단위는 다음 규칙으로 처리합니다:
  • 59초 이하: 표시하지 않습니다(생략). 예: 26분 1초 → "26분"
  • 60초 이상(단독 초 값): 분으로 올림(ceiling). 예: 90초 → "2분"
...
def _format_seconds_value(n: int) -> str:
    """n초를 표시 텍스트로 변환.

    - 59초 이하: 생략 → ""
    - 60초 이상: 분으로 올림(ceiling). 예: 90 → "2분", 3660 → "1시간 1분"
    """
    if n <= 0:
        return ""
    if n < 60:
        return ""
    total_minutes = math.ceil(n / 60)
...
def _replace_seconds(text: str) -> str:
    def repl(match: Match[str]) -> str:
        return _format_seconds_value(int(match.group(1)))

    result = SEC_WITH_UNIT_PATTERN.sub(repl, text)
