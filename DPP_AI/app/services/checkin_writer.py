...
입력: snapshot(오늘 데이터), behavior_breakdown, time_policy, user_configs, rewrite_instructions(선택)
...
입력(snapshot, behavior_breakdown, time_policy, user_configs)을 바탕으로 사용자가 인지해볼 만한 "사용 패턴 후보"만 제안합니다.
...
def generate_pattern_candidates(
    ...
    user_configs: dict,
    *,
    behavior_breakdown: dict | None = None,
