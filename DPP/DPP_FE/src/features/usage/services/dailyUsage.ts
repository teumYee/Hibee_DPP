...
import type { PostDailySnapshotV3Request } from "../../../services/api/checkin.api";
import type {
  PostUsageLogsRequest,
  UsageLogItem,
...
type UsageStatsModuleType = {
  ...
  getUsageForDayOffset?: (offsetDays: number) => Promise<UsageRow[]>;
  getLatestCompletedUnlockCount?: () => Promise<number>;
  setDayRolloverTime?: (value: string) => Promise<void> | void;
};
...
export async function syncNativeUsagePolicy(): Promise<void> {
  const mod = getUsageStatsModule();
  const dayRolloverTime =
    useAuthStore.getState().onboardingData.day_rollover_time || DEFAULT_DAY_ROLLOVER_TIME;
  if (!mod?.setDayRolloverTime) {
    return;
  }
  await Promise.resolve(mod.setDayRolloverTime(dayRolloverTime));
}

export async function readTodayUsageStats(): Promise<{
  rows: UsageRow[];
  unlockCount: number;
}> {
  const mod = getUsageStatsModule();
  ...
  return {
    ...
    unlockCount: typeof unlockCount === "number" ? unlockCount : 0,
  };
}

export async function readLatestCompletedUsageStats(): Promise<{
  rows: UsageRow[];
  unlockCount: number;
  logicalDate: string;
  capturedAt: string;
  ...
  const [rawRows, unlockCount] = await Promise.all([
    mod.getUsageForDayOffset ? mod.getUsageForDayOffset(0) : mod.getTodayUsage(),
    mod.getUnlockCount(),
  ]);
  ...
  return {
    rows: Array.isArray(rawRows) ? rawRows : [],
    unlockCount: typeof unlockCount === "number" ? unlockCount : 0,
    logicalDate: getLogicalDate(now),
    capturedAt,
  };
}

export function buildUsageLogsPayload(
  rows: UsageRow[],
  unlockCount: number,
  userId: number,
): PostUsageLogsRequest {
  const logs: UsageLogItem[] = rows.map((row) => ({
    user_id: userId,
    package_name: row.packageName,
    app_name: row.appName || "Unknown",
    usage_duration: Math.max(0, Math.round(row.usageTime || 0)),
    date: new Date(row.firstTimeStamp).toISOString(),
    start_time: new Date(row.firstTimeStamp).toISOString(),
    end_time: new Date(row.lastTimeStamp).toISOString(),
    unlock_count: 0,
    category_id: row.category ?? -1,
    is_night_mode: false,
    app_launch_count: Math.max(0, Math.round(row.appLaunchCount || 0)),
    max_continuous_duration: Math.max(0, Math.round(row.maxContinuousTime || 0)),
  }));
...
export function buildSnapshotPayload(
  ...
  const now = new Date();
  const logicalDate = options?.logicalDate || getLogicalDate(now);

  return {
    date: logicalDate,
    timezone,
    captured_at: options?.capturedAt || now.toISOString(),
